...
export default function Radar() {
    ...
    useEffect(() => {
        axios('ComfiredCompare.json').then(res => {
            comfiredData = res.filter(item=>item.updateTime === currentDate);
        }, [currentDate]).then(axios('Migrate.json').then(data => {
            migrateData = data.filter(item=>item.updateTime === currentDate);
        }, [currentDate]))
    ...
    function disabledDate(current) {
        return current < moment(new Date('2020/01/23')) || current > moment(new Date('2020/04/30'));
    }
